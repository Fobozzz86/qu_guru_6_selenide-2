...
public class Sandbox extends TestBase {
  ...
  @Test
  void closeCookiePopup() { //закрытие и проверка закрытия вспывающего окна cookie
  ...
  @Test
  void required() {
    // проверка при указании на поле логин/пароль мышки всплывающая подсказка required
    open("https://www.reddit.com/account/login/");
    $(".AnimatedForm").$("input#loginUsername").shouldHave(attribute("required"));
  }
  ...
  void pageObject() {
    ...
    new HomePO().title.shouldHave(text("My Balence"));
    new HomePO().showAccountBalance();

    new LoginPO2().loginWithUsernameAndPassword("user", "admin1234")
            .title.shouldHave(text("My Balence"));
           // .showAccountBalance(); вместо shouldHave(text("My Balence"))

  }

  // .clickAndHold - нажать и не отпускать клавишу мышки
  // .release - отпустить клавишу мышки
